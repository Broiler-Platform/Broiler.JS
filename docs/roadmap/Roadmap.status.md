...
Where the campaign stands, in full: the state of every phase and every item, what phase 2
changed, the committed Octane run and what may be read out of it, the noise band, the
conformance gate, the patch handoff — and §4, where the engine stands today.

> The evidence half of [`Roadmap.md`](Roadmap.md). **The plan document is the one to act
> from** — it carries the sequencing, each phase's verdict in one line, and the gates; this
> one carries the measurements behind them.
>
...
`patches/README.md`'s index carried no `Broiler.JS` row. The pin is
**`e5dc2610`**, and the index carried **ten** `Broiler.JS` rows — `0103` through `0112`. **All
ten have since been verified as landed and retired**: every one of their subjects is present in
the submodule log, which is the check this document's own preamble prescribes. *The obvious
check would have been wrong in both directions:* the added files of a sequentially-applied stack
exist in the pinned tree whether the whole stack landed or only its first patch, so file
existence can neither retire a patch nor keep one. **The only engine work now held outside the
pin is phase 5's item 2, as `patches/0115`.**
...
## Track two — the VM tier (phases 6–9): nothing measured

**Phases 6, 7, 8 and 9 have produced no measurements.** No item has been started, and each
phase is blocked on an entry measurement that has not been run:

| Phase | Blocked on | What that measurement decides |
|---|---|---|
| [6](Phase-6.status.md) | **6-0 · Scope the target** | Whether the track is needed at all — *including whether the IL path can be kept on the target platforms instead*, which would cancel phases 6–9 for the cost of an S |
| [7](Phase-7.status.md) | **7-0 · Profile the VM** | The VM's own band, its per-suite ratio to the IL path, and whether item 7-5's register machine is justified |
| [8](Phase-8.status.md) | **8-0 · Decompose the VM's time** | Which of phase 8's six items exist at all |
| [9](Phase-9.status.md) | **9-0 · Is tier-up worth it here?** | Whether promotion beats item 1-1's deferral — and it does *not* gate item 9-3, whose argument is phase 4's deoptimization gap |

**One thing about track two is recorded rather than measured**, and it is a file census
rather than a result: `Broiler.JavaScript.ExpressionCompiler` is a `System.Reflection.Emit`
IL writer with no second back end, and `Broiler.JavaScript.Portable` is **301 lines and 20
`double`-only opcodes**. Details in [`Phase-6.status.md`](Phase-6.status.md). It establishes
the starting point and **nothing about feasibility, cost or value.**
---
