...
**Nothing else here can be accepted until this is done**, and both source roadmaps
said so independently (§1.2). Discovery and correctness work may continue, but their
timings remain smoke or prioritization evidence. The phase contains no engine optimization;
controlled-lane and comparator work is performance infrastructure.

### Current protocol classification

The rows below preserve the campaign's historical measurements and contemporaneous gate
language. Under the current fail-closed [`Measurement.md`](Measurement.md) protocol:

- hosted Octane results are smoke, coverage and continuity evidence;
- the dirty developer-workstation run is prioritization evidence because its source content
  and immutable control were not captured;
- 7.5% is the historical harness repeatability guard, not a candidate regression threshold;
  and
- no RID yet has an accepted candidate/control bundle with exact rows, effective-setting
  attestation, semantic/test262 results and durable raw evidence.

This classification supersedes historical uses of “claim”, “gate” or “inside the band” below
without deleting the evidence that produced those statements.

### Harness readiness — **implemented**

| # | Item | State |
|---|---|---|
| **0-1** | Land the pending `Broiler.JS` patches | **Complete; the patch ledger is historical.** At the 2026-08-07 campaign snapshot the pointer was `685026c0` — `b3f53dcc` plus 0-9's probe corpus — and `cdb2fd41` (0048), `7ef80c03` (0046), and `8228b0da` (0047) were verified ancestors; the then-current `patches/README.md` also recorded 0049 for 1-2. At the 2026-08-22 reconciliation the pointer is `7fb17553`, `685026c0` remains an ancestor, and the aggregate `patches/` directory has been retired, so that deleted ledger is not a current work queue or reproducible link. Independently confirmed in the historical handoff: `ff819e06` refreshed the WPT baseline for a **net 36 fewer failures** after changes to `+`, `==`, the `for` head, and `eval` scoping |
...
| **0-4** | Quantify run-to-run stability | **Implemented for harness smoke.** `--repetitions`, median + spread, `--noise-band`, per-repetition logs, `flaky` status (§3.2). The configured band is not a candidate decision threshold |
...
|---|---|---|
| **0-6** | **Run the Octane workflow and commit refreshed results** | **Harness smoke satisfied.** The results in `tests/octane/results/linux-x64/` were regenerated by the workflow **2026-08-07** at the pinned pointer, against a same-machine Chromium 149.0.7827.55 and Jint 4.15.3. **17 of 17 scores, all 15 suites `ok` for all three engines, nothing errored or timed out.** Geomean 372 against Chromium's 58 718 (158×) and Jint's 577 (0.644×); spread 138×. It is the first committed run with `--repetitions 3`: 16 of 17 rows were inside the provisional 7.5% harness guard, median observed spread 3.0%, EarleyBoyer the lone 7.9%. See §4.2 for the retained table. A later hosted run may extend continuity history, but the pair is not an attributable candidate/control comparison |
| **0-7** | A `PropertyOperationBenchmarks` / `FunctionCallBenchmarks` comparison | **Run, not accepted** — `collect_phase0.py --profile baseline`, both classes plus the new probes, 2 repetitions, win-x64 at `b3f53dcc` on a developer workstation. It recorded `dirty: true` but not the patch/untracked inputs, and has no immutable control or attached semantic bundle. The stale 2026-07-16 artifacts no longer stand alone; the retained figures remain prioritization evidence |
| **0-8** | Decision-grade acceptance lanes on **win-x64, linux-x64, linux-arm64**, with applicable CPU-feature/GC arms and time/allocation/memory/size guardrails | **Not satisfied on any RID.** The historical win-x64 A/A leg had 25 of 62 timing metrics outside the provisional 7.5%, worst 56% (`FunctionCallBenchmarks.Native`), including five lifecycle timings. More importantly, the present path does not yet compare immutable candidate/control arms, enforce exact rows/all repetitions/all applicable metrics, attest effective settings, or attach semantic evidence. Linux x64 and Linux Arm64 controlled lanes are unavailable here |
...
| **0-10** | Pinned test262 over the historical four-manifest baseline | **Historical baseline done** — 8 313 tests, **zero engine failures** (§3.4). Getting there took three tooling fixes, below. Acceptance still requires each candidate/control decision to run and retain the current manifest named by its ownership entry |
| **0-11** | An `ownership.json` entry per item | **Implemented mapping; validation failing.** Fifteen were originally added, one per item rather than one per phase, bringing it to 36 total; 2-9 later added `shape-only-property-storage` for 37. The current file has **39** entries. The pre-existing `tiered-unboxed-locals` duplicates `numeric-local-doubles` and should be retired, and the configuration test still incorrectly fixes the total at 21. Replace that assertion with referential-integrity checks; an unknown or unowned comparison ID must fail before an acceptance run starts |
...
> **What this is not.** It is not current acceptance evidence. The workflow supplies 0-6's smoke
> coverage, while 0-7's controlled comparison and 0-8's RID matrix require evidence a container
> cannot produce. These results were therefore **not committed to `tests/octane/results/`**, which
> stays CI's to write. *(CI later wrote the 2026-08-03 results at the pin; their historical
> pass/fail reading agrees with this table: Richards 144.9×, DeltaBlue 460×.)* The full-suite
> numbers are **one repetition** and the harness says so. The table preserves the campaign's
> contemporaneous priority decision and same-machine five-repetition ratios; under the current
> protocol they are prioritization evidence, not an accepted candidate/control result.
...
Actions → Octane Benchmarks → Run workflow
  ...
  repetitions:     3            # the first run that reports a per-suite spread
```
...
**Current exit gate for phase 0:**

1. **Smoke coverage:** 17 of 17 scores, no suite timed out under its effective manifest
   budget, per-suite spread reported, and `comparison.md` carrying coverage/geomean/spread —
   ✅. The default floor is 180 s; Mandreel/zlib use 1,200/1,800 s overrides.
2. **Controlled stability:** workload/metric-specific A/A envelopes calibrated on each
   claimed lane, with hosted/container observations retained only as smoke — ❌.
3. **Fail-closed comparison:** immutable candidate/control identities, exact expected rows,
   every repetition and applicable resource guardrail, with one predeclared terminal decision
   plus a fresh confirmation for acceptance — ❌.
4. **Effective matrix and semantic bundle:** every claimed RID/CPU-feature/GC arm attests its
   effective settings and attaches the current ownership-mapped semantic/test262 results and
   durable checksummed raw evidence — ❌.
5. **Permanent probes:** 0-9 remains satisfied — ✅. Until 2–4 pass, later phase timings may
   prioritize work but cannot close a performance claim.
