...
- [ ] Structural / `SameValue` / parser categories needing per-file reproduction (`gap-673-triage-remaining`). Fixed so far: Cat 1 `delegating-yield-*` (`yield*` return value) and `concat-spreadable` (`@@isConcatSpreadable`), Cat 8 (`get`/`set constructor` in object literals), and Cat 10 (iterating primitives via prototype `@@iterator`) — see `Issue673Tests.cs`. Remaining: Cat 6 and the rest of Cat 1 (`Array.from` string-iterator override / Proxy trap order).
