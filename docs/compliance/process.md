...
4. For dynamic full-suite breadth, use `python scripts/compliance/run_test262.py --suite-ref <sha> --all-script-host-verifiable --shard-count <N> --shard-index <I>` or rely on `.github/workflows/test262-full-script-host.yml`; that workflow now runs on pull requests, pushes to `main`, and manual dispatch, discovers the entire current `test262` tree for the selected ref, and executes every currently script-host-verifiable file without maintaining manifests by hand.
5. Some upstream `test262` files are still intentionally excluded from that always-on CI run because the current raw script-host runner does not yet validate them correctly: negative-metadata tests need expected-error handling, host-harness-dependent tests need richer `$262`/host support, and `module` / `raw` tests need different host modes. The audit output and workflow summary report those exclusion counts so they remain visible.
6. The manifest smoke workflow remains available for focused reruns: `.github/workflows/test262.yml` executes every `scripts/compliance/test262-*.txt` file, which is still useful when promoting proven-clean directories or debugging a smaller subset.
