...
| `gap-673-triage-remaining` | Cats 1/6/8/10 — structural `deepEqual`, `SameValue(false,true)`, `Unexpected token … constructor`, `Value is not iterable` (see [triage](triage-issue-673.md#categories-1-6-8-10--require-per-file-reproduction)). Cat 1 `delegating-yield-*`, Cat 8 (object-literal `constructor` accessors), and Cat 10 (iterating primitives) **fixed**. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`; Cat 6 + rest of Cat 1 pending per-file reproduction | `Broiler.JS/Broiler.JavaScript.Runtime/JSIterator.cs`, `.../GeneratorsV2/GeneratorTypes.cs`, `Broiler.JS/Broiler.JavaScript.Parser/FastParser.ObjectLiteral.cs`, `Broiler.JS/Broiler.JavaScript.Runtime/JSPrimitive.cs` (done); `BuiltIns/Array/*` |
...
| `gap-675-overlap-673` | Issue #675 Problems 1, 2, 4–7 reproduce issue #673's categories (see [triage](triage-issue-675.md#problems-1-2-47--carried-over-from-issue-673)). `delegating-yield-*` (P1) and the finally-override (P3) are fixed; the rest map to the open `gap-673-*` batches above. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`, `Issue675Tests.cs` | see the `gap-673-*` rows above |
