...
| `gap-673-iterator-close` | `test/language/expressions/assignment/dstr/array-*-rtrn-close`, `test/language/statements/for-of/dstr/array-*-rtrn-close`, `staging/sm/generators/yield-iterator-close` (see [triage](triage-issue-673.md#category-2--iteratorclose-on-generator-return-high-confidence)) | New regression in `Broiler.JS/Broiler.JavaScript.Runtime.Tests` asserting `returnCount === 1` for a generator `return()` through a destructuring rest target | `Broiler.JS/Broiler.JavaScript.LinqExpressions/LinqExpressions/GeneratorsV2/GeneratorRewriter.cs` |
