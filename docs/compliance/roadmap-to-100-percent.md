...
| `gap-673-unicode-identifiers` | `test/language/identifiers/start-unicode-15.1.0/16.0.0/17.0.0(.js and -class.js)` (see [triage](triage-issue-673.md#categories-7--9--unicode-id_start-identifiers-and--private-names-high-confidence)) | New parser regression: one declaration per affected `ID_Start` code point | `Broiler.JS/Broiler.JavaScript.Parser/CharExtensions.cs` + pinned table in `Broiler.Unicode` |
...
| `gap-675-overlap-673` | Issue #675 Problems 1, 2, 4–7 reproduce issue #673's categories (see [triage](triage-issue-675.md#problems-2-47--carried-over-from-issue-673)). `delegating-yield-*` + `Array.from`-string-override (P1) and the finally-override (P3) are fixed; the rest map to the open `gap-673-*` batches above. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`, `Issue675Tests.cs` | see the `gap-673-*` rows above |
