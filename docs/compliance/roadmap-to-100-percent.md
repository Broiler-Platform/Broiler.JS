...
| `gap-673-triage-remaining` | Cats 1/6/8/10 — structural `deepEqual`, `SameValue(false,true)`, `Unexpected token … constructor`, `Value is not iterable` (see [triage](triage-issue-673.md#categories-1-6-8-10--require-per-file-reproduction)) | Per-file reproduction pending before promoting to dedicated batches | `BuiltIns/Array/*`, `Parser` class body, iterator protocol / `yield*` |
