...
| `gap-673-triage-remaining` | Cats 1/6/8/10 — structural `deepEqual`, `SameValue(false,true)`, `Unexpected token … constructor`, `Value is not iterable` (see [triage](triage-issue-673.md#categories-1-6-8-10--require-per-file-reproduction)). Cat 1 `delegating-yield-*` and Cat 8 (object-literal `constructor` accessors) **fixed**. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`; Cats 6/10 + rest of 1 pending per-file reproduction | `Broiler.JS/Broiler.JavaScript.Runtime/JSIterator.cs`, `Broiler.JS/Broiler.JavaScript.LinqExpressions/.../GeneratorsV2/GeneratorTypes.cs`, `Broiler.JS/Broiler.JavaScript.Parser/FastParser.ObjectLiteral.cs` (done); `BuiltIns/Array/*` |
