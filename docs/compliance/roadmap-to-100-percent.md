...
| `gap-673-iterator-close` | `test/language/expressions/assignment/dstr/array-*-rtrn-close`, `test/language/statements/for-of/dstr/array-*-rtrn-close`, `staging/sm/generators/yield-iterator-close` (see [triage](triage-issue-673.md#category-2--iteratorclose-on-generator-return-high-confidence)) — **fixed** | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs` (`GeneratorReturn_*`) | `Broiler.JS/Broiler.JavaScript.BuiltIns/Generator/JSGenerator.cs`, `Broiler.JS/Broiler.JavaScript.LinqExpressions/LinqExpressions/GeneratorsV2/GeneratorTypes.cs` (done) |
| `gap-673-finally-override` | `test/language/statements/try/S12.14_A9_T2`, `A10_T2`, `A11_T2`, `A12_T2` (see [triage](triage-issue-675.md#problem-3--finally-abrupt-completion-must-override-a-pending-throw-fixed)) — **fixed** (issue #675) | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue675Tests.cs` (`FinallyContinue_*`, `FinallyReturn_*`, `FinallyBreak_*`, `NonBranchingFinally_*`, `NestedBranchingFinally_*`, `CatchRethrow_*`) | `Broiler.JS/Broiler.JavaScript.ExpressionCompiler/Generator/ILCodeGenerator.VisitTryCatchFinally.cs`, `.../Generator/FinallyBranchScanner.cs`, `.../Core/ILWriter.cs`, `.../Core/ILTryBlock.cs` (done) |
| `gap-673-eval-injection` | `test/language/expressions/compound-assignment/S11.13.2_A6.4_T1` (and `A6.5/A6.7/A6.8`) (see [triage](triage-issue-673.md#category-5--direct-eval-var-injection--compound-assignment-lref-timing-high-confidence)) | New regression: direct-eval `var` injection + compound-assignment lref captured before RHS | engine/runtime eval semantics + identifier binding in `Broiler.JS/Broiler.JavaScript.Compiler` |
| `gap-673-unicode-identifiers` | `test/language/identifiers/start-unicode-15.1.0/16.0.0/17.0.0(.js and -class.js)` (see [triage](triage-issue-675.md#problems-6--7--unicode-15-1-id_start-coverage-fixed-via-net-10-bump)) — **fixed** (issue #675) by bumping every project from `net8.0` to `net10.0`; `char.GetUnicodeCategory` now uses Unicode 16 tables that recognise the 15.1/16.0 ID_Start additions | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue675Tests.cs` (`Unicode15_1_*`, `Unicode16_*`) | `Broiler.JS/Broiler.JavaScript.Parser/CharExtensions.cs` (now host-table-sufficient on net10; the pinned `Broiler.Unicode` table is no longer needed for ID_Start) |
| `gap-673-intl-range` | `test/intl402/DateTimeFormat/prototype/formatRange*`, `formatToParts/offset-timezone-correct` (see [triage](triage-issue-673.md#category-3--intldatetimeformat-range-formatting-medium-confidence)) | Extend `Broiler.JS/Broiler.JavaScript.Integration.Tests` `Intl` regressions | `Broiler.JS/Broiler.JavaScript.BuiltIns/Intl/JSIntl.cs` |
| `gap-673-triage-remaining` | Cats 1/6/8/10 — structural `deepEqual`, `SameValue(false,true)`, `Unexpected token … constructor`, `Value is not iterable` (see [triage](triage-issue-673.md#categories-1-6-8-10--require-per-file-reproduction)). Cat 1 `delegating-yield-*` + `Array.from`-string-override (issue #675), Cat 8 (object-literal `constructor` accessors), and Cat 10 (iterating primitives) **fixed**. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`, `Issue675Tests.cs`; Cat 6 + `Array/from_proxy.js` and `object/entries.js` pending per-file reproduction | `Broiler.JS/Broiler.JavaScript.Runtime/JSIterator.cs`, `.../GeneratorsV2/GeneratorTypes.cs`, `Broiler.JS/Broiler.JavaScript.Parser/FastParser.ObjectLiteral.cs`, `Broiler.JS/Broiler.JavaScript.Runtime/JSPrimitive.cs`, `Broiler.JS/Broiler.JavaScript.BuiltIns/String/JSString.cs` (done); `BuiltIns/Array/*` |
| `gap-675-missing-builtins` | Issue #675 Problems 8–10 — `test/built-ins/Set/prototype/{add,clear,forEach}/*`, `test/intl402/PluralRules/prototype/select/*`, `test/built-ins/Promise/withResolvers/*` (see [triage](triage-issue-675.md#problems-810--new-missing-or-incorrect-built-ins-fixed)) — **fixed** | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue675Tests.cs` | `Broiler.JS/Broiler.JavaScript.BuiltIns/Set/JSSet.cs`, `Broiler.JS/Broiler.JavaScript.BuiltIns/Set/JSWeakSet.cs`, `Broiler.JS/Broiler.JavaScript.BuiltIns/Intl/JSIntl.cs`, `Broiler.JS/Broiler.JavaScript.BuiltIns/Promise/JSPromiseStatic.cs` (done) |
| `gap-675-overlap-673` | Issue #675 Problems 1, 2, 4–7 reproduce issue #673's categories (see [triage](triage-issue-675.md#problems-2-47--carried-over-from-issue-673)). `delegating-yield-*` + `Array.from`-string-override (P1), the finally-override (P3) and the Unicode ID_Start coverage (P6/P7, fixed by the net10 bump) are done; only P2 (`Intl` range formatting), P4 (direct-eval var injection) and P5 (`SameValue(false,true)` mixed) remain. | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs`, `Issue675Tests.cs` | see the `gap-673-*` rows above |
