...
| `gap-673-triage-remaining` | Cats 1/6/8/10 — structural `deepEqual`, `SameValue(false,true)`, `Unexpected token … constructor`, `Value is not iterable` (see [triage](triage-issue-673.md#categories-1-6-8-10--require-per-file-reproduction)). Cat 1 `delegating-yield-*` **fixed** (`yield*` return value). | `Broiler.JS/Broiler.JavaScript.Integration.Tests/Issue673Tests.cs` (delegating-yield); rest pending per-file reproduction | `Broiler.JS/Broiler.JavaScript.Runtime/JSIterator.cs` + `Broiler.JS/Broiler.JavaScript.LinqExpressions/LinqExpressions/GeneratorsV2/GeneratorTypes.cs` (done); `BuiltIns/Array/*`, `Parser` class body |
