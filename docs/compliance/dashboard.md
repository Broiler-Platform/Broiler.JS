...
| test262 automated CI coverage | 2026-05-11 audit of pinned `tc39/test262` `ccaac100ff49d81e9ff47a75ff4c60e0bd3f262e`: 53,469 upstream files discovered, 47,420 currently script-host-verifiable, and 6,049 uniquely excluded by negative metadata, host-harness dependencies, or unsupported `module` / `raw` modes; CI is configured to shard and execute that full 47,420-file runnable set instead of the older 163-file manifest subset | `python scripts/compliance/audit_test262.py --suite-ref ccaac100ff49d81e9ff47a75ff4c60e0bd3f262e --manifest-glob 'scripts/compliance/test262-*.txt'`, `.github/workflows/test262-full-script-host.yml`, `.github/workflows/test262.yml`, and `python scripts/compliance/run_test262.py --suite-ref ccaac100ff49d81e9ff47a75ff4c60e0bd3f262e --all-script-host-verifiable --shard-count 8 --shard-index <0-7>` |
...
- That richer audit showed why the previous manifest-driven workflow stopped at 163 tests even though the raw script host could parse/run far more files: only 91 manifest entries had been promoted into CI, so the old workflow covered just 163 unique files out of 47,420 script-host-verifiable tests (0.34%). CI now uses the sharded full-suite workflow instead of that manifest-only subset.
...
2. **What currently blocks “all possible testcases”:**
   ...
   - A much broader run has to be sharded; CI now does that with the full script-host workflow, while the older serial manifest loop remains available only for focused smoke reruns.
3. **Incremental plan:**
...
### Full-suite automation now wired into CI
...
- `.github/workflows/test262-full-script-host.yml` now runs on pull requests, pushes to `main`, and manual dispatch, using that mode to fetch the selected `test262` ref and execute the full script-host-verifiable subset in 8 shards without maintaining manifest files by hand.
