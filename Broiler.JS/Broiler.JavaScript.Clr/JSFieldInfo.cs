...
internal readonly struct JSFieldInfo
{
    ...
    public JSFunction GenerateFieldGetter()
    {
        ...
        return new JSFunction(() =>
        {
            ...
            BExpression convertedThis = field.IsStatic ? null : JSValueToClrConverter.Get(ArgumentsBuilder.This(args), field.DeclaringType, "this");

            var body = ClrProxyBuilder.Marshal(BExpression.Field(convertedThis, field));
    ...
    public JSFunction GenerateFieldSetter()
    {
        ...
        return new JSFunction(() =>
        {
            ...
            var fieldExp = BExpression.Field(convert, field);

            // todo
            // not working for `char`
            var assign = BExpression.Assign(fieldExp, clrArg1).ToJSValue();
