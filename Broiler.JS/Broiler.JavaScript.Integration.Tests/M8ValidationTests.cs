...
public class M8ValidationTests
{
    ...
    public void M8_DocumentationFiles_Exist()
    {
        ...
        if (repoRoot != null)
        {
            ...
            Assert.Contains("Performance and deployment evidence", roadmap);
            Assert.Contains("scripts/compliance/test262-failures.txt", roadmap);
            Assert.Contains("HUMAN_REVIEW.md", roadmap);
            ...
            Assert.Contains("Current repository state", dashboard);
            ...
            Assert.Contains("scripts/compliance/test262-failures.txt", dashboard);
            Assert.Contains("--include-negative", dashboard);
            Assert.Contains("Release evidence", dashboard);
            ...
            Assert.Contains("Active semantic clusters", knownGaps);
            Assert.Contains("Host-coverage gaps", knownGaps);
            ...
            Assert.Contains("Broiler.JavaScript.Portable", performance);
        }
    ...
    private static string? FindRepoRoot(string startDir)
    {
        ...
        while (dir != null)
        {
            // A normal clone has a .git directory; a submodule checkout has a
            // .git pointer file. Both identify the component repository root.
